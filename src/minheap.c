...
//Refractored for dynamic sizing and modified for smallest to biggest sorting.
...
void max_heapify(struct binaryheap* heap,int i){
    ...
    if (left <= heap->lastelement && *(heap->index0+left)<*(heap->index0+smallest)){
        smallest=left;
    }
    // check if the right node is larger than the current node
    if (right <= heap->lastelement && *(heap->index0+right)<*(heap->index0+smallest)){
        smallest=right;
    }
    ...
    if (smallest!=i){
        swap(heap->index0+i,heap->index0+smallest);
        max_heapify(heap,smallest);
    }
}
// returns the minimal item of the heap
int get_min(struct binaryheap* heap)
{
    return *(heap->index0);
}
